...
x_i = F_i^{-1}(1-\lambda)
$$ Solve $\sum_{l=1}^{50} F_i^{-1}(1-\lambda) = K$ for $\lambda^{\star} \in (0,1)$ to get best allocation $x_i^{\star} = F_i^{-1}(1-\lambda^{\star})$
...
-   or perhaps $S(\mathbf{F}, \mathbf{y}) - \max(0, \sum y_i-K)$, perfomance against an oracle
